<footer class="page-footer blue darken-3">
    <div class="container">
        <div class="row">
            <div class="col l6 s12">
                <h5 class="white-text">Ποίοι είμαστε</h5>

                <p class="grey-text text-lighten-4">Είμαστε μια ομάδα φοιτητών του τμήματος Μηχανικών Πληροφοριακών
                    και Επικοινωνιακών Συστημάτων στο Πανεπιστήμιο Αιγαίου που κατασκευάσαν
                    το Samos Rentals ως μέρος ακαδημαϊκής εργασίας. Η εγγραφή και η
                    συμμετοχής σας είναι χρήσιμη για την υποστήριξη και συνέχιση του.</p>
            </div>
            <div class="col l3 m6 s12">
                <h5 class="white-text">Χρήσιμα</h5>
                <ul>
                    <li><a class="white-text" href="about.php">Σχετικά</a></li>
                    <li><a class="white-text" href="partners.php">Συνεργάτες</a></li>
                    <li><a class="modal-trigger white-text" href="#agreement">Όροι χρήσης</a></li>
                </ul>
            </div>
            <div class="col l3 m6 s12">
                <h5 class="white-text">Κοινωνικά Δίκτυα</h5>

                <div class="valign-wrapper center">
                    <a class="white-text" href="https://github.com/AegeanHawks">Github__</a>
                    <iframe src="https://ghbtns.com/github-btn.html?user=AegeanHawks&repo=SamosRentals&type=watch&count=true&size=medium&v=2"
                            frameborder="0" scrolling="0" width="160px" height="30px"></iframe>
                </div>
            </div>
        </div>
    </div>
    <div class="footer-copyright">
        <div class="container">
            <span class="left"> © <?php echo '2014' . '-' . date('Y'); ?>
                AegeanHawks, All rights reserved.</span>
            <span class="right">Κατασκευάσθηκε από τους <a class="brown-text text-lighten-3"
                                                           href="https://github.com/Rambou">Rambou</a> και
                <a class="brown-text text-lighten-3" href="https://github.com/Armageddonas">Armageddona</a></span>
        </div>
    </div>
</footer>

<?php
include 'agreement.php';
?>